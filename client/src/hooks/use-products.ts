...
export type ProductImportResult = z.infer<typeof api.products.importCatalog.responses[200]>;
...
export function useImportProducts() {
  ...
  return useMutation({
    mutationFn: async ({ file, markupPercent }: { file: File; markupPercent: number }) => {
      const formData = new FormData();
      formData.append("file", file);
      formData.append("markupPercent", String(markupPercent));

      const res = await fetch(api.products.importCatalog.path, {
        method: api.products.importCatalog.method,
        body: formData,
        credentials: "include",
      });
      ...
      if (!res.ok) {
        if (res.status === 400) {
          const error = api.products.importCatalog.responses[400].parse(await res.json());
          throw new Error(error.message);
        }
      ...
      return api.products.importCatalog.responses[200].parse(await res.json());
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: [api.products.list.path] }),
  });
}
